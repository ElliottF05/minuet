...
let rec accept listen_fd = 
  ...
  | exception Core_unix.Unix_error ((Core_unix.EINTR | Core_unix.ECONNABORTED), _, _) ->
      accept listen_fd
...
let rec read fd buf ?pos ?len () =
  ...
  | exception Core_unix.Unix_error ((Core_unix.EWOULDBLOCK | Core_unix.EAGAIN), _, _) -> 
      wait_readable fd;
      read fd buf ?pos ?len ()
  | exception Core_unix.Unix_error ((Core_unix.ECONNRESET | Core_unix.ETIMEDOUT) as err, _, _) -> 
      Error err
...
let rec write fd buf ?pos ?len () =
  ...
  | exception Core_unix.Unix_error ((Core_unix.ECONNRESET | Core_unix.ETIMEDOUT | Core_unix.EPIPE) as err, _, _) -> 
      Error err
